function [Offspring,FVal_offsp] = create_soffspring(parent1,parent2,Prey)
...
for i=1:n_subnets
    ...
    for j=1:n_blocks(i)                                   %Parfor loop performs xover and mutation between all blocks in parallel                     
          arr=parfor_sblock([subnet1{j},subnet2{j}],i,j); %arr is a cell array of size 2*1
          subnet(:,j)=arr;
    end
...
for i=1:2
    [err,complexity,coeff,modelout,in_end,height_n,nodes,max_height]=DFOReval(i,Offspring);
    FVal_offsp(i,:)=[err,complexity];
    Offspring(1,i).evaluate=0;
    Offspring(1,i).error = err;
    Offspring(1,i).complexity = complexity;
    Offspring(1,i).output=modelout;
    Offspring(1,i).in_end=in_end;
    Offspring(1,i).avg_subnet_ht=height_n;
    Offspring(1,i).coefficient=coeff;
    Offspring(1,i).nodes = nodes;
    Offspring(1,i).max_subnet_ht = max_height;
